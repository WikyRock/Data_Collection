    ...
    def set_single_state(self, sequence_number, width_to_set, height_to_set, exposure_time_to_set, offx_to_set, offy_to_set):
        try:
            ...
            if PySpin.IsAvailable(node_device_link_throughput_limit) and PySpin.IsWritable(node_device_link_throughput_limit):
                node_device_link_throughput_limit.SetValue(4300000000000)
            ...
            for i in range(self.NUM_IMAGES):
                try:
                    ...
                    if image_result.IsIncomplete():
                        ...
                        cv2.namedWindow("FLIR Camera", cv2.WINDOW_NORMAL)
                        cv2.resizeWindow("FLIR Camera", 640, 480) 
    ...
    def save_images_thread(self):
        ...
        with open(os.path.join('dataout', name_out, 'Master', 'TimeStamps.txt'), "w+") as f:
            for i in range(len(self.TimeStamps)):
                ...
                f.write('Timestamp:{}'.format(int(timestamp)/1000000000) + ' ' + '%d.jpg\n' % i)
